...
def create_interview_for_interviewer():
    ...
    cand_resp = client.post("/candidates/", json={
        "first_name": "John",
        "last_name": "Doe",
    ...
    int_resp = client.post("/interviews/", json={
...
def test_submit_feedback_not_assigned():
    ...
    login = client.post("/auth/login", json={"email": "other@nucleusteq.com", "password": "other123"})
...
def test_get_feedback_success():
    ...
    response = client.get(f"/feedbacks/interview/{interview_id}", headers={"Authorization": f"Bearer {int_token}"})
...
def test_get_feedback_not_found():
    ...
    response = client.get("/feedbacks/interview/unknown_id", headers={"Authorization": f"Bearer {int_token}"})
